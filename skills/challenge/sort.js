...
(window.ChallengeGames=window.ChallengeGames||{}).sort={
  ...
  render(host,r,ctx){
    ...
    host.querySelectorAll('.cg-bin').forEach(b=>b.onclick=()=>{ if(host.dataset.locked||!pick) return;
    ...
    check.onclick=()=>{
      ...
      setTimeout(()=>{ bad.forEach(c=>{ c.classList.remove('cg-no'); delete c.dataset.in; pool.appendChild(c); }); can(); },900);
    };
  }
};
